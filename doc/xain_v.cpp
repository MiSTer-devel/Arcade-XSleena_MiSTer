...
void xain_state::draw_sprites(bitmap_ind16 &bitmap,const rectangle &cliprect)
{
	for (int offs = 0; offs < m_spriteram.bytes();offs += 4)
	{
		...
		sx = 238 - m_spriteram[offs+3]; //sx = 238 - 255 = -17
		if (sx <= -7) sx += 256; //sx = -17 + 256 = 239
...
uint32_t xain_state::screen_update(screen_device &screen, bitmap_ind16 &bitmap, const rectangle &cliprect)
{
	...
	{
	case 0:
		m_bg_tilemaps[0]->draw(screen, bitmap, cliprect, TILEMAP_DRAW_OPAQUE, 0);//BG2
		m_bg_tilemaps[1]->draw(screen, bitmap, cliprect, 0,0);                   //BG1
		draw_sprites(bitmap,cliprect);                                           //SPR
		m_char_tilemap->draw(screen, bitmap, cliprect, 0,0);                     //TXT
		break;
	case 1:
